...
class MediaRepository @Inject constructor(
    ...
    val medium = mediaDao.queryFlow(opType = OpType.BACKUP, preserveId = DefaultPreserveId).distinctUntilChanged()
    val activatedCount = mediaDao.countActivatedFlow().distinctUntilChanged()

    fun queryFlow(name: String) = mediaDao.queryFlow(name = name).map { detail ->
        detail?.apply {
            entities = entities.filter { it.indexInfo.opType != OpType.BACKUP }
        }
    }

    fun getKeyPredicate(key: String): (MediaEntity) -> Boolean = { m ->
    ...
    private suspend fun calculateDataSize(m: MediaEntity) = rootService.calculateSize(m.path)
